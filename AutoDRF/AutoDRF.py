...
class AutoDRF(object):
    def __init__(self,ProjectPath, config={}, spacing=4):
        ...
        self.spacing= 4
        self.scriptFiles = ["serializers.py","views.py","urls.py","permissions.py"]
    ...
    def ProcessClassSingleScript(self, appPath, ClassName, ScriptFile, indenting=4):
        ...
        if ScriptFile=="serializers.py":
            ...
            for line in f:
                line = line.replace(" ","")
                if ClassName+"_ViewSet(viewsets.ModelViewSet)" ==  line:
                    found=True
            f.close()
            ...
            if (found==False):
                ...
                f.write(spacing+ "permission_classes = ["+ClassName+ "_Permission]\n")
                f.write(spacing+ "queryset = "+ClassName+ ".objects.all()\n")
                f.write(spacing+ "serializer_class = " + ClassName+"_Serializer\n")
                f.write("\n")
                f.close()

        elif ScriptFile=="permissions.py":
            #print("ProcessClassSingleScript Function - applying Class To Script : permissions.py" + ClassName)
            
            File = appfile.joinpath("api").joinpath("permissions.py")
            f = io.open(str(File.absolute()),'r')

            for line in f:
                line = line.replace(" ","")
                if ClassName +"_Permission(BasePermission)" ==  line:
                    found=True
            f.close()
            #print(found)
            if (found==False):
                ...
                f = io.open(str(File.absolute()),'a')
                f.write("\n")
                f.write("class " + ClassName +"_Permission(BasePermission): \n")		
		#view permission
                f.write(spacing+ "def has_permission(self, request, view):\n")
                f.write(spacing+ spacing + "return True \n")
                f.write("\n")

		#view object
                f.write(spacing+ "def has_object_permission(self, request, view, obj):\n")
                f.write(spacing+ spacing + "#if obj.creator == request.user:\n")
                f.write(spacing+ spacing + spacing + "#return True \n")
                f.write(spacing+ spacing + "return True \n")
                f.write("\n")
    ...
    def ProcessClassAllScripts(self,AppDir,ClassName,scripts=[]):
        if scripts==[]:
    ...
    def CreateAPIFiles(self, AppPath, indenting=4):
        ...
        if len(temp)== 0: #folder exist >> check for file
            ...
            files = ["serializers.py","views.py","urls.py","permissions.py"]
            for filename in files:
                ...
                if filetype == "serializers.py":
                    ...
                    file.write("from " + app.name+".api.permissions import * \n")
                elif filetype == "permissions.py":
                    file.write("from rest_framework.permissions import BasePermission, IsAuthenticated, SAFE_METHODS\n")
...
if __name__=='__main__':
	#path = "~/Desktop/moonspic/Project-Responsive/BE/v0/B/Platform"
	a = Path(os.getcwd())
	#E = AutoDRF(ProjectPath=path)
